...
static void decon_abd_pin_enable_interrupt(struct decon_device *decon, struct abd_pin *pin, bool on)
{
	...
	if (pin->name && !strcmp(pin->name, "pcd")) {
		atomic_set(&decon->bypass, (pin->level == pin->active_level) ? 1 : 0);
		decon_info("%s: bypass: %d\n", __func__, atomic_read(&decon->bypass));
	}
...
static int decon_abd_show(struct seq_file *m, void *unused)
{
	...
	abd_printf(m, "bypass: %d, lcdconnected: %d, lcdtype: %6X\n", atomic_read(&decon->bypass), dsim->priv.lcdconnected, lcdtype);
...
static int __init decon_abd_init(void)
{
	...
	if (decon->dt.out_type == DECON_OUT_DSI) {
		atomic_set(&decon->bypass, lcdtype ? 0 : 1);
